    ...
    std::shared_ptr<PostprocessorParameters<dim> const> parameters)
{
  ...
  for (auto cell : dof_handler.active_cell_iterators())
  {
    ...
    max_temperature =
        *std::max_element(temperature_values.begin(), temperature_values.end());
         ...
         it != this->debug_solution_fluxes.end(); ++it)
    {
      ...
      if (it->compare("heat_flux") == 0)
      {
        ...
        this->mp_values->get_values("thermal_conductivity", cell,
                                    thermal_conductivity_values);
        std::transform(thermal_conductivity_values.begin(),
                       thermal_conductivity_values.end(),
