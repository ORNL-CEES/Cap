...
{
  bool const valid_input = (time.size() == power.size()) &&
  ...
  while (it != end_it)
  {
    ...
    approximate_integral_with_trapezoidal_rule(
        std::next(time.begin(), first), std::next(time.begin(), last),
        std::next(power.begin(), first), std::next(energy.begin(), first), 0.0);
    it    = next;
    first = last;
  }
}
    ...
    std::vector<double> &energy_losses)
{
  ...
  approximate_integral_with_trapezoidal_rule(time.begin(), time.end(),
                                             heat_production.begin(),
                                             energy_losses.begin(), 0.0);
}
