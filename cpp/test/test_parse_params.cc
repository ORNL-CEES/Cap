...
BOOST_AUTO_TEST_CASE(test_parse_params)
{
  ...
  BOOST_CHECK_EQUAL(params.get<dealii::types::material_id>("some_material_id"),
                    dealii::types::material_id(3));
  BOOST_CHECK_EQUAL(params.get<dealii::types::material_id>("some_boundary_id"),
                    dealii::types::boundary_id(1));

  params.put("material_ids", "0,1,2,3");
  std::vector<dealii::types::material_id> material_ids =
      cap::to_vector<dealii::types::material_id>(
          params.get<std::string>(("material_ids")));
  ...
  BOOST_CHECK_THROW(params.get<double>("invalid_key"),
                    boost::property_tree::ptree_bad_path);
  BOOST_CHECK_THROW(params.get<double>("is_a_string"),
  ...
  std::vector<double> ones =
      cap::to_vector<double>(params.get<std::string>("ones"));
  std::vector<std::string> yes =
      cap::to_vector<std::string>(params.get<std::string>("yes"));
  std::vector<int> empty =
      cap::to_vector<int>(params.get<std::string>("empty"));
  ...
  std::vector<bool> vec_out =
      cap::to_vector<bool>(params.get<std::string>("boule"));
  BOOST_CHECK_EQUAL_COLLECTIONS(vec_in.begin(), vec_in.end(), vec_out.begin(),
                                vec_out.end());

  params.put("always_true", "    true,true, true  ");
  std::vector<bool> always_true =
      cap::to_vector<bool>(params.get<std::string>("always_true"));
  ...
  BOOST_CHECK_THROW(params.get<double>(path),
                    boost::property_tree::ptree_bad_data);
  ...
  BOOST_CHECK_EQUAL(pi_to_string.compare(0, precision + 1,
                                         params.get<std::string>(path), 0,
                                         precision + 1),
                    0);
}
