...
//std::vector<double>
//compute_signal_to_noise_ratio(std::vector<double> const & data, std::vector<int> const & components)
//{
...
//measure_impedance_old(std::shared_ptr<cap::EnergyStorageDevice> dev, std::shared_ptr<boost::property_tree::ptree const> database)
//{
...
//
////    std::cout<<"voltage signal-to-noise ratio = "<<compute_signal_to_noise_ratio(voltage, dummy_components)[0]<<"\n";
////    std::cout<<"current signal-to-noise ratio = "<<compute_signal_to_noise_ratio(current, dummy_components)[0]<<"\n";
//
...
void impedance_spectroscopy(std::shared_ptr<cap::EnergyStorageDevice> dev, std::shared_ptr<boost::property_tree::ptree const> database, std::ostream & os = std::cout)
{
    ...
    for (double frequency = frequency_upper_limit; frequency >= frequency_lower_limit; frequency /= std::pow(10.0, 1.0/steps_per_decade))
    {
        ...
        {
//            double               frequency = std::get<0>(data);
//            std::complex<double> impedance = std::get<1>(data);
            double               frequency = data.first;
            std::complex<double> impedance = data.second;
