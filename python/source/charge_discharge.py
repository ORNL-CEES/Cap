...
class Charge(MultiStage):
    def __init__(self,ptree):
        ...
        if voltage_finish:
            ...
            other.put_string('stage_1.criterion_0.end_criterion','current_less_than')
            other.put_double('stage_1.criterion_0.current_limit',charge_voltage_finish_current_limit)
            other.put_string('stage_1.criterion_1.end_criterion','time')
            other.put_double('stage_1.criterion_1.duration',charge_voltage_finish_max_time)
        else:
...
class Discharge(MultiStage):
    def __init__(self,ptree):
        ...
        if discharge_mode=='constant_current'or discharge_mode=='galvanostatic':
        ...
        elif discharge_mode=='constant_voltage'or discharge_mode=='potentiostatic':
        ...
        if discharge_stop_at_1=='voltage_less_than':
            other.put_string('stage_0.end_criterion','compound')
            other.put_string('stage_0.logical_operator','or')
            other.put_string('stage_0.criterion_0.end_criterion',discharge_stop_at_1)
            discharge_voltage_limit=ptree.get_double('discharge_voltage_limit')
            other.put_double('stage_0.criterion_0.voltage_limit',discharge_voltage_limit)
        else:
            raise RuntimeError("invalid discharge_stop_at_1 criterion '"+discharge_stop_at_1+"'")
        try:
            discharge_stop_at_2=ptree.get_string('discharge_stop_at_2')
            other.put_string('stage_0.criterion_1.end_criterion',discharge_stop_at_2)
            discharge_max_duration=ptree.get_double('max_discharge_duration')
        except RuntimeError:
...
class CyclicChargeDischarge(MultiStage):
    def __init__(self,ptree):
        ...
        if start_with=='charge':
            ...
            raise RuntimeError("Invalid first step '"+start_with+"' in CyclicChargeDischarge")
