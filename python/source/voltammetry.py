...
class CyclicVoltammetry:
    ...
    def run(self, device, data=None):
        ...
        for cycle in range(self.cycles):
            steps += ramp(device, data, self.scan_limit_1, self.scan_rate,
                          self.step_size)
            steps += ramp(device, data, self.scan_limit_2, self.scan_rate,
                          self.step_size)
        steps += ramp(device, data, self.final_voltage, self.scan_rate,
                      self.step_size)
