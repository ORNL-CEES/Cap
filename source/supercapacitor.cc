...
public:
    ...
    std::unique_ptr<EnergyStorageDevice> build(boost::mpi::communicator const & comm, boost::property_tree::ptree const & ptree)
    {
        int const dim = ptree.get<int>("dim");
        if (dim == 2)
            return std::unique_ptr<SuperCapacitor<2>>(new SuperCapacitor<2>(comm, ptree));
        else if (dim == 3)
            return std::unique_ptr<SuperCapacitor<3>>(new SuperCapacitor<3>(comm, ptree));
        else
