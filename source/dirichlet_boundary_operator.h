...
public:
    ...
    void apply_rhs_correction(dealii::Vector<double> & right_hand_side)
    {
        if (!zero_diagonal)
            ...
            if (!rhs_set.empty())
                throw dealii::ExcMessage("rhs_set is not empty but zero_diagonal is true");
        ...
        if (symmetric_correction)
            ...
            if (!rhs_add.empty())
                throw dealii::ExcMessage("rhs_add is not empty but symmetric_correction is false");
     }
    ...
    void apply_matrix_correction(dealii::SparseMatrix<double> & matrix)
    {
        ...
        for (auto const & dof : boundary_values)
        {
            ...
            if (!(dof_index < n_dofs))
                throw std::runtime_error("invalid dof index (probably should make this debug only but it helps getting rid of silly warning non used variable)");
            // for each boundary dof:
    
            // set entries of this line to zero except for the diagonal
            // entry
            for (dealii::SparseMatrix<double>::iterator
                 p = matrix.begin(dof_index);
            ...
            if (this->symmetric_correction)
            {
                     ...
                     q != matrix.end(dof_index); ++q)
                {
                    ...
                    const dealii::SparseMatrix<double>::iterator
                    p = dealii::Utilities::lower_bound(matrix.begin(row)+1,
                                                       matrix.end(row),
                                                       dof_index,
                    ...
                    Assert ((p != matrix.end(row))
                            &&
                            (p->column() == dof_number),
                            dealii::ExcMessage("This function is trying to access an element of the "
