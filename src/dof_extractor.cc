...
void
DoFExtractor::
reset(dealii::ComponentMask const & row_mask, 
      dealii::ComponentMask const & column_mask, 
      unsigned int const            dofs_per_cell)
{
    unsigned int const n_components = row_mask.size();
    Assert(column_mask.size() == n_components, dealii::StandardExceptions::ExcDimensionMismatch(row_mask.size(), column_mask.size()));
...
DoFExtractor::
extract_row_indices(std::vector<dealii::types::global_dof_index> const & dofs_in) const
{
...
std::vector<dealii::types::global_dof_index> 
DoFExtractor::
extract_column_indices(std::vector<dealii::types::global_dof_index> const & dofs_in) const
{
...
template dealii::FullMatrix<double> DoFExtractor::extract_matrix(dealii::FullMatrix<double> const & ) const;
